 ...
 */
package org.eclipse.leshan.client.javacoap.endpoint.stdio;
...
public class OpensslProcessTransport extends StreamBlockingTransport {
    ...
    public static ProcessBuilder createProcess(String certPemFile, InetSocketAddress destination, boolean isDtls,
            String cipherSuite) throws IOException {
        String sessIn = new File("openssl-session.tmp").exists() ? "-sess_in openssl-session.tmp " : "";
        String dtls = isDtls ? "-dtls " : "";

        String cmd = String.format(
                "%s s_client -crlf -ign_eof -connect %s -cert %s -cipher %s %s -sess_out openssl-session.tmp %s -quiet",
                getOpensslBinPath(), toString(destination), certPemFile, cipherSuite, sessIn, dtls).replace("  ", " ");

        LOGGER.info("Running " + cmd);

        return new ProcessBuilder(cmd.split(" ")).redirectError(ProcessBuilder.Redirect.INHERIT);
    }

    public static ProcessBuilder createProcess(Pair<String, Opaque> psk, InetSocketAddress destination, boolean isDtls,
            String cipherSuite) throws IOException {
        String dtls = isDtls ? "-dtls " : "";

        String cmd = String.format(
                        "%s s_client -crlf -ign_eof -connect %s -psk_identity %s -psk %s -cipher %s %s -quiet",
