...
import org.eclipse.leshan.client.javacoap.LwM2mClientCoapResource;
 ...
 */
public class JavaCoapRootResource extends LwM2mClientCoapResource {

    protected DownlinkRequestReceiver requestReceiver;
    protected ClientEndpointToolbox toolbox;

    public JavaCoapRootResource(IdentityHandlerProvider identityHandlerProvider,
    ...
    @Override public void handleGET(CoapExchange exchange) {
        ...
        BootstrapDiscoverResponse response = requestReceiver.requestReceived(identity,
                new BootstrapDiscoverRequest(URI, coapRequest)).getResponse();
    ...
    @Override public void handleFETCH(CoapExchange exchange) {
        Request coapRequest = exchange.advanced().getRequest();
        ServerIdentity identity = getServerOrRejectRequest(exchange, coapRequest);
        if (identity == null)
            return;
    ...
    @Override public void handleIPATCH(CoapExchange exchange) {
        // Manage Read Composite request
        Request coapRequest = exchange.advanced().getRequest();
        ServerIdentity identity = getServerOrRejectRequest(exchange, coapRequest);
        if (identity == null)
        ...
        Map<LwM2mPath, LwM2mNode> nodes = toolbox.getDecoder()
                .decodeNodes(coapRequest.getPayload(), contentFormat, null, toolbox.getModel());

        WriteCompositeResponse response = requestReceiver.requestReceived(identity,
                new WriteCompositeRequest(contentFormat, nodes, coapRequest)).getResponse();
        if (response.getCode().isError()) {
            exchange.respond(toCoapResponseCode(response.getCode()), response.getErrorMessage());
        } else {
            exchange.respond(toCoapResponseCode(response.getCode()));
    ...
    @Override public void handleDELETE(CoapExchange exchange) {
        ...
        BootstrapDeleteResponse response = requestReceiver.requestReceived(identity,
                new BootstrapDeleteRequest(URI, coapRequest)).getResponse();
        exchange.respond(toCoapResponseCode(response.getCode()), response.getErrorMessage());
    }
}
